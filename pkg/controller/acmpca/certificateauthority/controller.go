...
func (e *external) Create(ctx context.Context, mgd resource.Managed) (managed.ExternalCreation, error) {
	...
	meta.SetExternalName(cr, aws.StringValue(response.CertificateAuthorityArn))
...
func (e *external) Update(ctx context.Context, mgd resource.Managed) (managed.ExternalUpdate, error) { // nolint:gocyclo
	...
	if len(cr.Spec.ForProvider.Tags) > 0 {
		tags := make([]*types.Tag, len(cr.Spec.ForProvider.Tags))
		for i, t := range cr.Spec.ForProvider.Tags {
			tags[i] = &types.Tag{Key: aws.String(t.Key), Value: aws.String(t.Value)}
		}
	...
	_, err := e.client.UpdateCertificateAuthority(ctx, &awsacmpca.UpdateCertificateAuthorityInput{
		...
		Status:                  types.CertificateAuthorityStatus(aws.StringValue(cr.Spec.ForProvider.Status)),
	})
...
func (e *external) Delete(ctx context.Context, mgd resource.Managed) error {
	...
	if response != nil {
		if response.CertificateAuthority.Status == types.CertificateAuthorityStatusActive {
			_, err = e.client.UpdateCertificateAuthority(ctx, &awsacmpca.UpdateCertificateAuthorityInput{
				CertificateAuthorityArn: aws.String(meta.GetExternalName(cr)),

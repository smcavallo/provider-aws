...
func isUpToDateEnvironment(cr *svcapitypes.Function, obj *svcsdk.GetFunctionOutput) bool {
	...
	return compareMapStringStringPointer(envVars, awsVars)
}

// compareSliceStringPointer checks if 2 []*string contain the same values
func compareSliceStringStringPointer(cr []*string, obj []*string) bool {
	if len(cr) != len(obj) {
		return false
	}
	for i, v := range cr {
		if aws.StringValue(v) != aws.StringValue(obj[i]) {
			return false
		}
	}
	return true
...
// compareMapStringStringPointer checks if 2 map[string]*string contain the same keys and values
func compareMapStringStringPointer(cr map[string]*string, obj map[string]*string) bool {
	for k, v := range cr {
		if aws.StringValue(obj[k]) == aws.StringValue(v) {
			continue
		}
		return false
	}
	for k, v := range obj {
		if aws.StringValue(cr[k]) == aws.StringValue(v) {
			continue
		}
		return false
	}
...
func isUpToDateSecurityGroupIDs(cr *svcapitypes.Function, obj *svcsdk.GetFunctionOutput) bool {
	...
	return compareSliceStringStringPointer(securityGroupIDs, awsSecurityGroupIDs)
}
...
func GenerateUpdateFunctionCodeInput(cr *svcapitypes.Function) *svcsdk.UpdateFunctionCodeInput {
	...
	f0.SetFunctionName(*cr.Spec.ForProvider.FunctionName)
